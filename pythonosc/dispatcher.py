...
class Dispatcher(object):
    ...
    def map_raw(self, address, handler, *args):
        self._raw[address].append(Handler(handler, list(args)))
    ...
    def handlers_for_address(self, address_pattern):
        ...
        for addr, handlers in self._map.items():
            if pattern.match(addr) or (('*' in addr) and re.match(addr.replace('*', '[^/]*?/*'), address_pattern)):
                yield from handlers
                matched = True
    ...
    def raw_handlers_for_address(self, address_pattern):
        """yields Handler namedtuples matching the given OSC pattern."""
        # First convert the address_pattern into a matchable regexp.
        # '?' in the OSC Address Pattern matches any single character.
        # Let's consider numbers and _ "characters" too here, it's not said
        ...
        for addr, handlers in self._raw.items():
            if pattern.match(addr) or (('*' in addr) and re.match(addr.replace('*', '[^/]*?/*'), address_pattern)):
                yield from handlers
                matched = True
